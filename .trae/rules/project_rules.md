# 抖音UI定制器项目规则

本文档定义了抖音UI定制器项目的开发规范、工作流程和最佳实践，所有参与该项目的开发人员都应严格遵守。

## 1. 项目概述

- **项目名称**: 抖音UI定制器
- **项目URL**: https://github.com/sutchan/douyin_tool
- **主要开发者**: Sut
- **项目类型**: 浏览器用户脚本(UserScript)
- **技术栈**: JavaScript, Vite, CSS

## 2. 目录结构规范

项目采用标准的前端项目结构，所有文件和目录应遵循以下规范：

### 2.1 核心目录结构

```
douyin_tool/
├── src/                 # 源代码目录
│   ├── styles/          # 样式文件目录
│   ├── utils/           # 工具函数目录
│   ├── config.js        # 配置管理
│   ├── index.js         # 项目入口文件
│   ├── main.js          # 主程序逻辑
│   └── ui_manager.js    # UI管理器
├── dist/                # 构建输出目录
├── docs/                # 文档目录
├── build/               # 构建脚本目录(多平台构建源码)
├── .trae/               # 项目配置目录
│   └── rules/           # 规则文件目录
├── .gitignore           # Git忽略配置
├── package.json         # 项目配置与依赖
├── vite.config.js       # Vite构建配置
└── README.md            # 项目说明文档
```

### 2.2 文件组织原则

- **源代码**: 所有源代码必须放在`src/`目录下
- **构建产物**: 构建生成的文件必须放在`dist/`目录下
- **文档**: 所有文档必须放在`docs/`目录下
- **构建源码**: 多平台构建的源代码放在`build/`目录下

## 3. 代码规范

### 3.1 JavaScript代码规范

- **代码风格**: 遵循JavaScript标准风格，使用ES6+语法
- **缩进**: 使用2个空格进行缩进
- **分号**: 语句结束必须使用分号
- **引号**: 字符串使用双引号("")，除非嵌套需要
- **命名规范**:
  - 变量/函数: 小驼峰命名法 `camelCase`
  - 类/构造函数: 大驼峰命名法 `PascalCase`
  - 常量: 全大写并使用下划线分隔 `UPPER_SNAKE_CASE`

### 3.2 注释规范

- **函数注释**: 所有函数必须包含JSDoc格式的注释
- **行内注释**: 复杂逻辑必须添加行内注释
- **模块注释**: 每个模块文件开头必须有模块说明
- **注释语言**: 统一使用中文进行注释

### 3.3 错误处理与日志

- **错误处理**: 关键操作必须使用try-catch进行错误处理
- **日志记录**: 使用`logger.js`模块进行日志记录，禁止直接使用`console.log`
- **事件通知**: 使用`eventEmitter.js`模块进行组件间通信

## 4. 版本控制规范

### 4.1 Git分支规范

- **main**: 主分支，用于发布稳定版本
- **develop**: 开发分支，集成开发中的功能
- **feature/**: 功能分支，命名格式为`feature/功能名称`
- **bugfix/**: 修复分支，命名格式为`bugfix/问题描述`

### 4.2 提交规范

- **提交信息**: 使用清晰简洁的中文描述
- **提交粒度**: 一次提交应对应一个完整功能或一个问题修复
- **提交格式**: 建议使用`[类型] 描述`格式，如`[新增] 添加暗黑模式支持`

### 4.3 版本号规范

- **版本格式**: 采用语义化版本号`major.minor.patch`
- **版本更新规则**:
  - `major`: 不兼容的API变更
  - `minor`: 向下兼容的功能性新增
  - `patch`: 向下兼容的问题修复
- **版本同步**: 所有代码文件的版本号必须同步更新
- **构建版本**: 每次构建自动升级最小版本号(patch)

## 5. 构建流程规范

### 5.1 构建准备

- **依赖安装**: 开发前必须执行`npm install`
- **缓存清理**: 每次构建前必须清理工作区缓存文件

### 5.2 构建命令

- **开发构建**: `npm run dev`
- **生产构建**: `npm run build`
- **预览构建**: `npm run preview`

### 5.3 构建要求

- **构建产物**: 构建产物必须生成在`dist/`目录
- **文件名**: 构建后的用户脚本文件名为`douyin_ui_customizer.user.js`
- **版本信息**: 构建时自动更新`@version`标签

## 6. 文档规范

### 6.1 文档类型

- **技术文档**: 存放在`docs/05-技术文档/`目录
- **API文档**: 存放在`docs/05-技术文档/api.md`
- **README**: 根目录下的`README.md`作为项目概览文档

### 6.2 文档要求

- **文档语言**: 统一使用中文
- **格式**: 使用Markdown格式
- **更新频率**: 功能变更时必须同步更新相关文档

## 7. 测试与质量保证

### 7.1 代码质量

- **代码检查**: 提交前必须通过`npm run lint`检查
- **代码格式化**: 使用`prettier`保持代码风格一致

### 7.2 功能测试

- **功能验证**: 新功能必须经过手动测试验证
- **兼容性测试**: 确保在主流浏览器中正常工作

## 8. 部署与发布

### 8.1 发布流程

1. 确认代码通过所有测试
2. 更新版本号
3. 执行生产构建
4. 更新版本历史文档
5. 提交发布版本

### 8.2 版本历史

- 每次构建都必须更新版本历史文档
- 记录功能变更、问题修复和性能优化

## 9. 项目特有规则

### 9.1 用户脚本规范

- **元数据**: 必须包含完整的UserScript元数据
- **权限声明**: 仅声明必要的GM_*权限
- **匹配规则**: 明确声明脚本运行的URL范围

### 9.2 性能优化

- **事件处理**: 使用防抖(debounce)和节流(throttle)优化事件处理
- **资源加载**: 延迟加载非关键资源
- **DOM操作**: 最小化DOM操作频率，使用文档碎片

### 9.3 样式管理

- **主题支持**: 实现浅色和深色主题支持
- **样式隔离**: 确保样式不影响页面其他元素

## 10. 安全规范

- **代码安全**: 避免使用可能导致XSS的代码
- **数据存储**: 敏感数据必须加密存储
- **API使用**: 遵循浏览器安全策略，避免跨域问题

---

本文档由Sut创建并维护，最后更新时间：2024年4月15日